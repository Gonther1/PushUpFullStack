...
namespace API.Controllers
{
    public class TipoPagoController : BaseController
    {
        ...
        {
            var entity = await _unitOfWork.TiposPagos.GetAllAsync();
            return _mapper.Map<List<TipoPagoDto>>(entity);
        }
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<Tipopago>> Post(TipoPagoDto entityDto)
        {
            var entity = _mapper.Map<Tipopago>(entityDto);
        /*
        ...
        */
            _unitOfWork.TiposPagos.Add(entity);
        ...
        {
            var entity = await _unitOfWork.TiposPagos.GetByIdAsync(id);
            if (entity == null)
            {
                return NotFound();
            }
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<TipoPagoDto>> Put(int id, [FromBody] TipoPagoDto entityDto)
        {
            var entity = _mapper.Map<Tipopago>(entityDto);
            if (entity.Id == 0)
            {
        ...
        */
            entityDto.Id = entity.Id;
            _unitOfWork.TiposPagos.Update(entity);
            await _unitOfWork.SaveAsync();
            return entityDto;
        }
        [HttpDelete("{id}")]
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            _unitOfWork.TiposPagos.Remove(entity);
            await _unitOfWork.SaveAsync();
            return NoContent();
        }
    }
